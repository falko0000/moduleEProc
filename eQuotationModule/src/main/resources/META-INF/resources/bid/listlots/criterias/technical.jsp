...
<%
   ...
   List<Criteria> criterias = Collections.emptyList();
	...
	if(!criterias.isEmpty())	
       criteriasWeight = CriteriasWeightLocalServiceUtil.getCriteriasWeight(spisok_lotov_id, EQuotationConstants.CRITERIA_TECHNICAL);
			...
			<c:if test = "<%=bin[bin.length-5]!= 0 %>">
		  	...
		  	<c:if test = "<%=bin[bin.length-6]!= 0 %>">
		  		...
		  		<aui:col md="<%=String.valueOf(12-2*bin[bin.length-5]) %>">
					...
					<aui:input label="Technical total weight in percent" name="<%=prefix + "_totalWeight"%>" type="number" value="<%=criteriasWeight.getCriterias_weight() %>" max="100" min="0" required="true"/>
