...
%>
  <div id=<%=prefix+"-auto-fields-container" %>>
  	...
  	<div class="lfr-form-row lfr-form-row-inline">
		...
		<aui:input label="Qualification total weight in percent" name="<%=prefix + "_totalWeight1"%>" type="number" value="0" max="100" min="0" required="true" />
		   </aui:col>
		   </c:if>
			...
			<c:if test = "<%=bin[bin.length-5]!= 0 %>">
		  	...
		  	<c:if test = "<%=bin[bin.length-6]!= 0 %>">
		  		...
		  		<aui:col md="<%=String.valueOf(12-2*bin[bin.length-5]) %>">
					...
					<aui:input label="Qualification total weight in percent" name="<%=prefix + "_totalWeight1"%>" type="number" value="<%=criteriasWeight.getCriterias_weight() %>" max="100" min="0" required="true"/>
