           ...
           <%=LanguageUtil.format(request, "lot-number", lotInfo) %>
            ...
            <%=LanguageUtil.get(request, "ranking-quotation") %>
             <%=LanguageUtil.format(request,"at-time-opening", lotInfo[0])%>
              ...
              <%@ include file="/commissiontab/evaluation/opening/list_suppliers.jspf" %>
                 
          <%} %>  
                <%=LanguageUtil.format(request, "automated-claims","zakupki.gov.tj")%>
                <%=LanguageUtil.get(request, "claims-fed")%>
                <%=LanguageUtil.format(request, "submission-claim","complaint@zakupki.gov.tj")%>
               
                  <%=LanguageUtil.format(request, "operator-system",izvewenija.getUserName())%>
                  <%=LanguageUtil.get(request, "to-take")%>